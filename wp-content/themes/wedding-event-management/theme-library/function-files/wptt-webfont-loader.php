...
if ( ! class_exists( 'WPTT_WebFont_Loader' ) ) {
	...
	class WPTT_WebFont_Loader {
		 ...
		 * @access public
		 * @since 1.1.0
		 * @param string $wedding_event_management_url The remote URL.
		 */
		public function __construct( $wedding_event_management_url = '' ) {
			$this->remote_url = $wedding_event_management_url;
		...
		public function get_local_files_from_css() {
			...
			foreach ( $font_files as $font_family => $files ) {
				...
				foreach ( $files as $wedding_event_management_url ) {

					// Get the filename.
					$filename  = basename( wp_parse_url( $wedding_event_management_url, PHP_URL_PATH ) );
					...
					if ( file_exists( $font_path ) ) {
						...
						if ( isset( $stored[ $wedding_event_management_url ] ) ) {
							continue;
						}

						// Add file to the cache and change the $changed var to indicate we need to update the option.
						$stored[ $wedding_event_management_url ] = $font_path;
						$change         = true;
					...
					$tmp_path = download_url( $wedding_event_management_url );
			...
			if ( $change ) {
				...
				foreach ( $stored as $wedding_event_management_url => $path ) {
					if ( ! file_exists( $path ) ) {
						unset( $stored[ $wedding_event_management_url ] );
					}
				}
		 ...
		 * @access public
		 * @since 1.0.0
		 * @param string $wedding_event_management_format The format to be used. Use "woff" or "woff2".
		 * @return void
		 */
		public function set_font_format( $wedding_event_management_format = 'woff2' ) {
			$this->font_format = $wedding_event_management_format;
		}
		 ...
		 * @access protected
		 *
		 * @param string $wedding_event_management_url The URL.
		 *
		 * @return string
		 */
		protected function get_absolute_path( $wedding_event_management_url ) {

			// If dealing with a root-relative URL.
			if ( 0 === stripos( $wedding_event_management_url, '/' ) ) {
				$wedding_event_management_parsed_url = parse_url( $this->remote_url );
				return $wedding_event_management_parsed_url['scheme'] . '://' . $wedding_event_management_parsed_url['hostname'] . $wedding_event_management_url;
			}

			return $wedding_event_management_url;
		}
	}
}
...
if ( ! function_exists( 'wptt_get_webfont_styles' ) ) {
	 ...
	 * @since 1.0.0
	 *
	 * @param string $wedding_event_management_url    The URL of the remote webfont.
	 * @param string $wedding_event_management_format The font-format. If you need to support IE, change this to "woff".
	 *
	 * @return string Returns the CSS.
	 */
	function wptt_get_webfont_styles( $wedding_event_management_url, $wedding_event_management_format = 'woff2' ) {
		$wedding_event_management_font = new WPTT_WebFont_Loader( $wedding_event_management_url );
		$wedding_event_management_font->set_font_format( $wedding_event_management_format );
		return $wedding_event_management_font->get_styles();
	}
}
...
if ( ! function_exists( 'wptt_get_webfont_url' ) ) {
	 ...
	 * @return string Returns the CSS.
	 */
	function wptt_get_webfont_url( $wedding_event_management_url, $wedding_event_management_format = 'woff2' ) {
		$wedding_event_management_font = new WPTT_WebFont_Loader( $wedding_event_management_url );
		$wedding_event_management_font->set_font_format( $wedding_event_management_format );
		return $wedding_event_management_font->get_url();
	}
}